...
void AddrinfoInjectSetResponse(int rv, int num_results, const struct AddrinfoResult *results)
{
    ...
    struct addrinfo_mock_data* response = malloc(sizeof(struct addrinfo_mock_data));
...
static int invoke_system_getaddrinfo(const char *node, const char *service,
