                                       ...
                                       void *data)
{
    ...
    raft = &f->cluster.rafts[leader_id - 1];

    server = &raft->configuration.servers[f->cluster.n - 1];
    munit_assert_int(server->id, ==, f->cluster.n);
...
static MunitResult test_remove(const MunitParameter params[], void *data)
{
    ...
    rv = raft_remove_server(raft, leader_id % f->cluster.n + 1);
    munit_assert_int(rv, ==, 0);

    rv = test_cluster_run_until(&f->cluster, test_cluster_committed_2, 2000);
    munit_assert_int(rv, ==, 0);

    munit_assert_int(raft->configuration.n, ==, f->cluster.n - 1);
