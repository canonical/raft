                        ...
                        struct uv_stream_s *stream)
{
    s->uv = uv;
    s->id = id;
    s->address = MyHeapMalloc(strlen(address) + 1);
    if (s->address == NULL) {
        return RAFT_NOMEM;
    }
                            ...
                            uv_buf_t *buf)
{
    ...
    if (s->buf.len == 0) {
        ...
        if (s->payload.len == 0) {
            ...
            s->header.base = MyHeapMalloc(s->header.len);
            if (s->header.base == NULL) {
        ...
        s->payload.base = MyHeapMalloc(s->payload.len);
        if (s->payload.base == NULL) {
            /* Setting all buffer fields to 0 will make read_cb fail with
             * ENOBUFS. */
            memset(buf, 0, sizeof *buf);
                       ...
                       struct uv_stream_s *stream)
{
    ...
    server = MyHeapMalloc(sizeof *server);
    if (server == NULL) {
        rv = RAFT_NOMEM;
        goto err;
    }
