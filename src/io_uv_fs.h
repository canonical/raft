 ...
 */
#define RAFT_IO_UV_FS_MAX_PATH_LEN 1024

/**
 * Maximum length of a file name.
 */
#define RAFT_IO_UV_FS_MAX_FILENAME_LEN 128

/**
 * Maximum length of the data directory path.
 */
#define RAFT_IO_UV_FS_MAX_DIR_LEN \
    (RAFT_IO_UV_FS_MAX_PATH_LEN - RAFT_IO_UV_FS_MAX_FILENAME_LEN - 1)
...
struct raft_io_uv_fs; /* Forward declaration */

/**
 * Callback for asynchronous file system operations.
 */
typedef void (*raft_io_uv_fs_cb)(struct raft_io_uv_fs *req);

/**
 * Request object for asynchronous file system operations.
 */
struct raft_io_uv_fs
{
    void *data;                   /* user data */
    struct raft_io_uv_file *file; /* file handle */
    int status;                   /* request result */
    struct uv_work_s work;        /* pending work thread request, if any */
    raft_io_uv_fs_cb cb;          /* callback to invoke upon completion */
