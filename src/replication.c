...
{
    if (r->fsm->version == 1 ||
        (r->fsm->version > 1 && r->fsm->snapshot_async == NULL)) {
        snapshotClose(s);
        return;
    }
...
static int takeSnapshot(struct raft *r)
{
    ...
    bool sync_snapshot = r->fsm->version < 2 || r->fsm->snapshot_async == NULL;
    if (sync_snapshot) {
        /* putSnapshot will clean up config and buffers case of error */
