...
static int raft__uv_file_create_sync_dir(const char *path);
static int raft__uv_file_create_set_direct_io(struct raft__uv_file *f);
...
/**
 * Callback run after @raft__uv_file_write_work_cb has returned. It's run in the
 * main loop thread.
 */
static void raft__uv_file_write_after_work_cb(uv_work_t *work, int status)
{
