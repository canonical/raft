...
static int load_n(struct raft_io *io,
                  const char *dir,
                         ...
                         struct io_uv__metadata *metadata)
{
    ...
    rv = load_n(io, dir, 1, &metadata1);
    if (rv != 0) {
        return rv;
    }

    rv = load_n(io, dir, 2, &metadata2);
    if (rv != 0) {
        return rv;
    }
                          ...
                          const struct io_uv__metadata *metadata)
{
    ...
    if (fd == -1) {
        errorf(io, "open %s: %s", path, uv_strerror(-errno));
        return RAFT_ERR_IO;
    }
    ...
    if (rv == -1) {
        errorf(io, "write %s: %s", path, uv_strerror(-errno));
        return RAFT_ERR_IO;
    }
    ...
    if ((size_t)rv < sizeof buf) {
        errorf(io, "write %s: only %d bytes written", path, rv);
        return RAFT_ERR_IO;
    };
                  ...
                  struct io_uv__metadata *metadata)
{
    ...
    if (rv == -1) {
        errorf(io, "read %s: %s", path, uv_strerror(-errno));
        close(fd);
        return RAFT_ERR_IO;
    }
    if (rv != sizeof buf) {
    ...
    if (rv != 0) {
        errorf(io, "decode %s: %s", path, raft_strerror(rv));
        return RAFT_ERR_IO;
    }

    /* Sanity checks that values make sense */
    if (metadata->version == 0) {
        errorf(io, "metadata %s: version is set to zero", path);
        return RAFT_ERR_IO;
    }
                  ...
                  struct io_uv__metadata *metadata)
{
    ...
    for (i = 0; i < 2; i++) {
        metadata->version++;
        rv = io_uv__metadata_store(io, dir, metadata);
        if (rv != 0) {
            return rv;
        }
    }
